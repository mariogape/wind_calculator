...
def create_dense_urban_cfd_test_dataset(
    *,
    source_dir: str | Path,
    output_dir: str | Path,
    window_size: int = 512,
    ...
    with rasterio.open(terrain_path) as terrain_src, rasterio.open(buildings_path) as buildings_src, rasterio.open(
        ...
        terrain_valid_clip = terrain_valid[
            int(window.row_off) : int(window.row_off + window.height),
            int(window.col_off) : int(window.col_off + window.width),
        ]
        building_values_clip = building_values[
            int(window.row_off) : int(window.row_off + window.height),
            int(window.col_off) : int(window.col_off + window.width),
        ].astype(np.float32)
